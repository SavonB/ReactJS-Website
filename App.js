...
class App extends Component {
    constructor(props) {
        ...
        this._search_by_cookingMethod= this._search_by_cookingMethod(this); 
    }
...
/* reassign titlequery on input change */
    ...
    handleClick() {
        this.setState({
            ...
            cookingMethod_query: document.getElementsByTagName("input")[0].value
        });
    }
    ...
    _search_by_cookingMethod() {
        let x = 0;
        for (let i = 0; i < recipesJSON.recipes.length; i++) {
            if (recipesJSON.recipes[i].cookingMethod.toLowerCase().includes(this.state.cookingMethod_query.toLowerCase())) {
                console.log(true)
                x = recipesJSON.recipes[i];
                return x;
            } else {
                console.log(this.state.cookingMethod_query.toLowerCase())
                console.log(recipesJSON.recipes[i].cookingMethod.toLowerCase())

                x= recipesJSON.recipes[0];
            }
        }
        return x;
    }
    ...
    render() {
        ...
        let j = recipesJSON.recipes[0];
        j= this._search_by_cookingMethod
        ...
        return (
            <div className="App">
                ...
                <button onClick={this.handleClick}>Search by Cooking Method</button>
                <div>
                ...
                <Recipes recipe_to_display={q} recipe_to_display2={j} title_query={ this.state.title_query}cookingMethod_query={this.state.cookingMethod_query}  buttondisplay={this.state.display_state} inputdisplay={this.state.input_display_state}
